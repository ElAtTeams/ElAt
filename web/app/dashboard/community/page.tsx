    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
            ...
            <Button
              ...
              onClick={
                selectedMember
                  ? handleBackToList
                  : () => {
                      /* Handle back to previous page if not member detail */
                    }
              }
            >
              <ChevronLeft className="h-5 w-5" />
            </Button>
            <h1 className="text-xl font-bold text-white">{selectedMember ? selectedMember.name : "Topluluğum"}</h1>
          </div>
            ...
            <Card className="border-0 shadow-sm">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="flex items-center">
                        ...
                        {community.members.toLocaleString()} üye • {radius} km çapında
                      </CardDescription>
                    </div>
                  </div>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-4 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-emerald-600">{community.stats.gifts}</div>
                    <p className="text-sm text-muted-foreground">İyilik</p>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-blue-600">{community.stats.asks}</div>
                    <p className="text-sm text-muted-foreground">Talep</p>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-amber-500">{community.stats.gratitudes}</div>
                    <p className="text-sm text-muted-foreground">Teşekkür</p>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-indigo-600">{community.stats.connections}</div>
              ...
              <CardContent>
                <div className="flex items-center space-x-4">
                  <Avatar className="h-16 w-16">
                    <AvatarImage src={community.builder.avatar || "/placeholder.svg"} />
                    <AvatarFallback>AK</AvatarFallback>
                  </Avatar>
                  <div>
                    <h4 className="font-semibold">{community.builder.name}</h4>
                    <p className="text-sm text-muted-foreground">
                      {community.builder.posts} gönderi • {community.builder.raves} takdir
                    </p>
                  </div>
                </div>
                <p className="mt-4 text-sm text-muted-foreground">{community.builder.bio}</p>
              </CardContent>
            </Card>
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent className="p-0">
                  <div className="divide-y">
                    {community.membersList.map((member: any) => (
                      <div
                        key={member.id}
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex-1">
                            <h4 className="font-medium">
                              {member.name} <span className="text-sm text-muted-foreground">({member.location})</span>
                            </h4>
                            <p className="text-sm text-muted-foreground">
                              {member.posts} gönderi • Son aktivite: {member.active}
                            </p>
                          </div>
                ...
                <CardFooter className="p-0">
                  <Button variant="ghost" className="w-full h-12 rounded-none text-emerald-600 hover:text-emerald-700">
            ...
            {activeTab === "activity" && (
              <Card className="border-0 shadow-sm">
                <CardContent className="p-8 text-center">
                  ...
                  <h3 className="text-xl font-semibold mb-2">Henüz Aktiviteniz Yok</h3>
                  <p className="text-muted-foreground mb-6">Topluluğunuzdaki son aktiviteler burada görünecek</p>
                  <Button>
            ...
            {activeTab === "map" && (
              <Card className="border-0 shadow-sm">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-6">Yakınınızdaki topluluk üyelerini haritada görün</p>
                  <Button>
          ...
          <div className="p-4 space-y-4">
            ...
            <Card className="border-0 shadow-sm">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-xl font-semibold mb-1">Merhaba, Ben {selectedMember.name}</h3>
                ...
                <Button variant="outline" className="rounded-full bg-transparent">
                  <Star className="h-4 w-4 mr-2" /> {selectedMember.name.split(" ")[0]} hakkında yorum yap
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <h3 className="font-semibold">Topluluk: {selectedMember.location}</h3>
              </CardHeader>
              <CardContent>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-40">
        <div className="flex justify-around items-center py-2">
          ...
          {/* Topluluk */}
          <Link href="/dashboard/community" className="flex flex-col items-center justify-center text-emerald-600">
