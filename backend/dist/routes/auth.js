...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const user = await User_1.default.create({ firstName, lastName, email, password: hash });
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET || 'elatsecret', { expiresIn: '7d' });
...
router.put('/complete-profile', auth_1.authMiddleware, async (req, res) => {
    const { userId, phone, address, city, district, bio, interests, isOnboardingComplete } = req.body;
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        const resetToken = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET || 'elatsecret', { expiresIn: '1h' });
